...
from predict_has_delay import predict_has_delay
...
def predict_revenue_loss(
    ...
    if delay_hours_override is not None:
        ...
        query_date = None
        if journey_start is not None:
            if isinstance(journey_start, dict):
                query_date = journey_start.get("date")
            elif hasattr(journey_start, "isoformat"):
                query_date = journey_start.isoformat()
        classification = predict_has_delay(weather_dict, query_date=query_date)
        result["classification"] = classification

        if not classification["has_delay"]:
            delay_pct = 0.0
            delay_hours = 0.0
            result["model1_results"] = {"Ridge": 0, "Random Forest": 0, "XGBoost": 0}
            result["model1_skipped"] = True
        else:
