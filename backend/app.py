...
def api_source():
    ...
    return jsonify({"name": "Tracy, CA", **TRACY_DEPOT})
...
def api_route_delays():
    ...
    try:
        ...
        query_date = None
        if journey_start and isinstance(journey_start, dict):
            query_date = journey_start.get("date")

        # Seasonal diesel price factor (US average — winter heating-oil demand + summer driving peak)
        SEASONAL_DIESEL = {
            1: 1.10, 2: 1.10, 3: 1.04, 4: 1.02, 5: 1.03,
            6: 1.05, 7: 1.07, 8: 1.05, 9: 1.03, 10: 1.02,
            11: 1.06, 12: 1.10,
        }
        journey_month = 6
        if query_date:
            try:
                journey_month = int(str(query_date)[5:7])
            except (ValueError, TypeError, IndexError):
                pass
        seasonal_fuel_factor = SEASONAL_DIESEL.get(journey_month, 1.0)
        ...
        for idx, r in enumerate(routes_in):
            ...
            for i, (lat, lng) in enumerate(sampled):
                ...
                if feats:
                    # Classify this pitstop's weather
                    cls = predict_has_delay(feats, query_date=query_date)
                    all_pitstop_probs.append(cls["probability"])
            ...
            # Ensemble: average of all three models (used as the recommended prediction)
            avg_delays["Ensemble"] = round(
                sum(avg_delays[m] for m in ("Ridge", "Random Forest", "XGBoost")) / 3, 2
            )
            # Fuel cost: base 6.5 MPG diesel at $4.00/gal, adjusted for weather efficiency
            # loss (each 1% delay ≈ 0.4% MPG reduction, floored at 60% of baseline) and
            # seasonal diesel price variation.
            distance_miles = (r.get("distance_m") or 0) / 1609.34
            ensemble_delay_pct = avg_delays.get("Ensemble", 0)
            effective_mpg = 6.5 * max(0.6, 1.0 - ensemble_delay_pct * 0.004)
            fuel_price = 4.00 * seasonal_fuel_factor
            fuel_cost_usd = round((distance_miles / effective_mpg) * fuel_price, 2)
            out_routes.append({
        ...
        # Aggregate classification across all pitstops
        if all_pitstop_probs:
            avg_prob = sum(all_pitstop_probs) / len(all_pitstop_probs)
            max_prob = max(all_pitstop_probs)
            severe_count = sum(1 for p in all_pitstop_probs if p >= 0.5)
            severe_ratio = severe_count / len(all_pitstop_probs)
            has_delay = severe_ratio >= 0.2 or max_prob >= 0.7
            classification = {
                "has_delay": has_delay,
                "probability": round(avg_prob, 4),
                "max_probability": round(max_prob, 4),
                "severe_pitstops": severe_count,
                "total_pitstops": len(all_pitstop_probs),
                "model_name": "XGBoost",
            }
        else:
            classification = {
                "has_delay": False, "probability": 0, "max_probability": 0,
                "severe_pitstops": 0, "total_pitstops": 0, "model_name": "XGBoost",
            }

        return jsonify({"routes": out_routes, "classification": _enrich_classification(classification)})
    except Exception as e:
        import traceback
        return jsonify({
...
def _risk_level(has_delay, max_prob):
    if not has_delay:
        return "Low"
    if max_prob >= 0.85:
        return "Critical"
...
def _enrich_classification(c):
    """Add risk_level and severe_ratio to a classification dict."""
    total = c.get("total_pitstops", 0)
    severe = c.get("severe_pitstops", 0)
    c["severe_ratio"] = round(severe / total, 4) if total else 0
    c["risk_level"] = _risk_level(c.get("has_delay", False), c.get("max_probability", 0))
    return c


@app.route("/api/route-weather-risk", methods=["POST"])
def api_route_weather_risk():
    """
    Lightweight classification-only endpoint: sample pitstops, fetch weather,
    classify weather risk. Does NOT run delay models.
    Body: { "routes": [ ... ], "date": "YYYY-MM-DD" (optional) }
    """
    from datetime import date as _date
    try:
        data = request.get_json() or {}
        routes_in = data.get("routes") or []
        query_date = data.get("date") or _date.today().isoformat()
    except Exception as e:
        return jsonify({"error": "Invalid JSON body", "detail": str(e)}), 400
    if not routes_in:
    ...
    try:
        ...
        for r in routes_in:
            if not isinstance(r, dict):
                continue
            points = decode_polyline(r.get("polyline") or "")
            for lat, lng in sample_route_points_by_distance(points):
                feats = get_weather_features_for_pitstop(lat, lng)
                if feats:
                    cls = predict_has_delay(feats, query_date=query_date)
                    all_pitstop_probs.append(cls["probability"])
        ...
        if all_pitstop_probs:
            ...
            classification = {
                ...
                "max_probability": round(max_prob, 4),
                "severe_pitstops": severe_count,
                "total_pitstops": total,
                "severe_ratio": round(severe_ratio, 4),
                "risk_level": _risk_level(has_delay, max_prob),
                "model_name": "XGBoost",
            }
        else:
            classification = {
                "has_delay": False, "probability": 0, "max_probability": 0,
                "severe_pitstops": 0, "total_pitstops": 0,
                "severe_ratio": 0, "risk_level": "Low", "model_name": "XGBoost",
            }
        ...
        import traceback
        return jsonify({
            "error": "Weather risk assessment failed",
            "detail": str(e),
            "traceback": traceback.format_exc(),
        }), 500
...
def api_classify():
    """Pre-Model 1: classify whether weather conditions indicate severe weather (likely delay)."""
    try:
        data = request.get_json() or {}
        weather = data.get("weather") or data
        query_date = data.get("date") or data.get("journey_start_date")
        model_name = str(data.get("model", "xgboost")).lower()

        if model_name == "all":
            results = predict_has_delay_all(weather, query_date=query_date)
            return jsonify({"classifications": results})
        else:
            result = predict_has_delay(weather, model_name=model_name, query_date=query_date)
            return jsonify(result)
    except FileNotFoundError as e:
        return jsonify({"error": "Classifier not found. Run build_classifier.py first.", "detail": str(e)}), 503
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_route_loss():
    ...
    try:
        ...
        # Pre-Model 1: classify whether severe weather is likely
        query_date = None
        if journey_start and isinstance(journey_start, dict):
            query_date = journey_start.get("date")
        elif isinstance(journey_start, str):
            query_date = journey_start
        classification = predict_has_delay(weather, query_date=query_date)

        if not classification["has_delay"]:
            m1_results = {"Ridge": 0, "Random Forest": 0, "XGBoost": 0}
            delay_pct = 0.0
            model1_skipped = True
        else:
