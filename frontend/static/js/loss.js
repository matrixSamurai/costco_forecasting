(function () {
  ...
  // Auto-populate from query params and auto-calculate when arriving from the route page
  (function () {
    var params = new URLSearchParams(window.location.search);
    if (!params.has("delay_hours")) return;

    var PARAM_FIELDS = [
      "lettuce_type", "organic", "quantity_lb", "month",
      "delay_hours", "delay_temp_f", "transit_distance_km", "pre_delay_consumption_pct",
    ];
    PARAM_FIELDS.forEach(function (key) {
    ...
    // Update harvest hint text only (don't overwrite fields already set from params)
    var m = parseInt(params.get("month") || "", 10);
    if (m && harvestHint) {
      var info = HARVEST_REGIONS[m];
      if (info) harvestHint.textContent = "Source: " + info.region;
    }
